# -*- coding: utf-8 -*-
"""诊断右栏AI指挥台内容: msgs有几条消息/智能栏几个按钮/空白原因"""
import asyncio, json, sys, io, subprocess, time, os
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
PORT = 9243
EDGE = r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
async def main():
    proc = subprocess.Popen([EDGE, "--headless=new", f"--remote-debugging-port={PORT}",
        "--user-data-dir=" + os.environ['LOCALAPPDATA'] + r"\Temp\ep43",
        "--no-first-run", "--disable-sync", "--window-size=1280,820", "about:blank"],
        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    await asyncio.sleep(5)
    import websockets, urllib.request
    tabs = json.loads(urllib.request.urlopen(f"http://127.0.0.1:{PORT}/json").read())
    ws_url = next(t["webSocketDebuggerUrl"] for t in tabs if t["type"] == "page")
    async with websockets.connect(ws_url, max_size=10**7) as ws:
        await ws.send(json.dumps({"id": 0, "method": "Page.enable"}))
        await ws.send(json.dumps({"id": 99, "method": "Page.navigate", "params": {"url": "http://127.0.0.1:8899/"}}))
        await asyncio.sleep(8)
        expr = r'''(()=>{
const msgs=document.getElementById('msgs');
const mcount=msgs?msgs.children.length:-1;
const mtext=msgs?msgs.innerText.slice(0,150):'';
const bar=document.getElementById('smartBar');
const bcount=bar?bar.children.length:-1;
const btext=bar?bar.innerText.replace(/\s+/g,'|').slice(0,200):'';
const chat=document.querySelector('.chatbox');
const cr=chat?chat.getBoundingClientRect():null;
const ev=document.getElementById('evtBox');
const er=ev?ev.getBoundingClientRect():null;
return 'msgs条数:'+mcount+' | 智能栏按钮:'+bcount+' | chatbox高:'+(cr?Math.round(cr.height):'?')+' | 事件高:'+(er?Math.round(er.height):'?')+' | 栏内容:'+btext
})()'''
        await ws.send(json.dumps({"id": 1, "method": "Runtime.evaluate",
            "params": {"expression": expr, "returnByValue": True}}))
        t0 = time.time()
        while time.time() - t0 < 12:
            try:
                m = json.loads(await asyncio.wait_for(ws.recv(), timeout=4))
            except asyncio.TimeoutError:
                continue
            if m.get("id") == 1:
                print(m["result"]["result"].get("value"))
                break
    proc.kill()
asyncio.run(main())
